...
├── effectsConfig.ts          # 动画特效配置（樱花、光标尾迹）
...
| `backgroundWallpaper.ts` | 背景壁纸配置（壁纸模式、图片/视频、横幅文字、水波纹等） |
| `booknavConfig.ts` | 书签导航配置（页面设置与书签分组数据） |
...
| `dynamicConfig.ts` | 动态页面配置（页面标题、描述、评论开关、每页显示数量、Memos 数据源） |
| `effectsConfig.ts` | 动画特效配置（樱花、光标尾迹） |
| `cursorConfig.ts` | 自定义光标配置（`.cur` 资源路径映射、用户开关） |
...
| `musicConfig.ts` | 音乐播放器配置（Meting API / 本地音乐、导航栏和侧边栏开关、歌词显示） |
...
- `displaySettingsConfig.ts` 的视图设置面板在本仓库默认开启（`enable: true`）；除了修改配置里的 `enable`，也可以在部署平台（Vercel / Cloudflare 等）设置环境变量 `PUBLIC_DISPLAY_SETTINGS=true` 开启，无需改动配置文件；环境变量优先级更高，取值 `true/1/on/yes` 开启、`false/0/off/no` 关闭
