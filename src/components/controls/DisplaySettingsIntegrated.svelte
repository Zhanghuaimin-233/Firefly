...
let cursorTrailEnabled = $state(true);
...
<div id="display-setting" class="float-panel float-panel-closed absolute transition-all w-80 right-4 px-3 pt-0 pb-3 max-h-[80vh] overflow-y-auto custom-scrollbar" data-floating-panel data-floating-panel-trigger="display-settings-switch" inert aria-hidden="true">
	...
	<!-- Effects Tab: Sakura / Cursor / Cursor Trail -->
	{#if activeTab === "effects"}
		...
		<div class="">
			<div class="section-title">
				...
				<button aria-label="Reset to Default" class="btn-regular rounded-md active:scale-90"
						class:opacity-0={(sakuraEnabled === defaultSakuraEnabled || !isSakuraSwitchable) && (cursorEnabled === defaultCursorEnabled || !isCursorSwitchable) && (cursorTrailEnabled === defaultCursorTrailEnabled || !isCursorTrailSwitchable)}
						class:pointer-events-none={(sakuraEnabled === defaultSakuraEnabled || !isSakuraSwitchable) && (cursorEnabled === defaultCursorEnabled || !isCursorSwitchable) && (cursorTrailEnabled === defaultCursorTrailEnabled || !isCursorTrailSwitchable)}
						disabled={(sakuraEnabled === defaultSakuraEnabled || !isSakuraSwitchable) && (cursorEnabled === defaultCursorEnabled || !isCursorSwitchable) && (cursorTrailEnabled === defaultCursorTrailEnabled || !isCursorTrailSwitchable)}
						aria-hidden={(sakuraEnabled === defaultSakuraEnabled || !isSakuraSwitchable) && (cursorEnabled === defaultCursorEnabled || !isCursorSwitchable) && (cursorTrailEnabled === defaultCursorTrailEnabled || !isCursorTrailSwitchable) ? "true" : undefined}
						onclick={() => { if (isSakuraSwitchable) { sakuraEnabled = defaultSakuraEnabled; setSakuraEnabled(defaultSakuraEnabled); } if (isCursorSwitchable) { cursorEnabled = defaultCursorEnabled; setCursorEnabled(defaultCursorEnabled); } if (isCursorTrailSwitchable) { cursorTrailEnabled = defaultCursorTrailEnabled; setCursorTrailEnabled(defaultCursorTrailEnabled); } }}>
					<div class="text-(--btn-content)">
						<Icon icon="fa7-solid:arrow-rotate-left" class="text-[0.75rem]"></Icon>
					</div>
				</button>
